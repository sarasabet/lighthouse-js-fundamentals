...
const checkAir = function (samples, threshold) {
  ...
  let polutionLevel = 0;// number of poluted loctaions 
    for ( i =0; i < sample; i ++){
    if( samples[i] === 'dirty' ){
      polutionLevel ++;
    }
  }
  const polutionPercent =  polutionLevel/sample; //calculate percentage of polution
  if (polutionPercent <= threshold) {
    return 'clean';
  } else {
    return 'poluted'
  }
